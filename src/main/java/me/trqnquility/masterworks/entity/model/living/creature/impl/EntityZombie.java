...
public final class EntityZombie extends EntityCreature {
    ...
    public void tick() {
        ...
        List<PathNode> path = state.getAStar().findPath(entityPosition().getX() / 32, entityPosition().getY() / 32, state.getPlayer().getPosition().getX() / 32, state.getPlayer().getPosition().getY() / 32);
